...
import { Timestamp } from "https://www.gstatic.com/firebasejs/9.22.1/firebase-firestore-compat.js";
    ...
    saveButton.addEventListener('click', async () => {
        ...
        for (const row of jsonData) {
            try {
                ...
                // Add metadata
                deviceData.createdAt = Timestamp.now();
                ...
                inspectionData.vizsgalatIdopontja = inspectionData.vizsgalatIdopontja ? Timestamp.fromDate(new Date(inspectionData.vizsgalatIdopontja)) : Timestamp.now();
                inspectionData.kovetkezoIdoszakosVizsgalat = inspectionData.kovetkezoIdoszakosVizsgalat ? Timestamp.fromDate(new Date(inspectionData.kovetkezoIdoszakosVizsgalat)) : null;
                inspectionData.deviceId = deviceRef.id;
                inspectionData.createdAt = Timestamp.now();
